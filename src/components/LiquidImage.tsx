...
function LiquidMaterial({ imageUrl, isHovered }: { imageUrl: string, isHovered: boolean }) {
  const url = useMemo(() => imageUrl + (imageUrl.includes('?') ? '&' : '?') + 'webgl=true', [imageUrl]);
...
export function LiquidImage({ src, isHovered, className }: { src: string, isHovered: boolean, className?: string }) {
  return (
