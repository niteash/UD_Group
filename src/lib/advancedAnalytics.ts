...
class AdvancedAnalytics {
  ...
  private initTrackers() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (!isInteractive && target.tagName !== 'HTML' && target.tagName !== 'BODY') {
        this.trackEvent({
          category: 'UX Issue',
          action: 'Dead Click',
          label: target.tagName + (target.className ? `.${target.className}` : ''),
   ...
   */
  public trackVideo(action: 'play' | 'pause' | 'complete', videoId: string, progress?: number) {
    this.trackEvent({
      category: 'Video',
...
export function useVisibilityTracking(componentName: string) {
  ...
  const trackVisibility = (entries: IntersectionObserverEntry[]) => {
